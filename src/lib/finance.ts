...
export function sessionAmounts(minutes: number, rate: number, commissionRate = 0, category: WorkSession['time_category'] = 'billable', commissionCap: number | null = null, commissionAlreadyUsed = 0) { const gross = category === 'billable' ? Math.round((minutes / 60) * rate * 100) / 100 : 0; const calculatedCommission = Math.round(gross * (commissionRate / 100) * 100) / 100; const remainingCap = commissionCap === null ? null : Math.max(0, Number(commissionCap) - Number(commissionAlreadyUsed)); const commission = remainingCap === null ? calculatedCommission : Math.min(calculatedCommission, Math.round(remainingCap * 100) / 100); return { gross, commission, net: Math.round((gross - commission) * 100) / 100 }; }
export function missionCommissionUsed(projectId: string, sessions: WorkSession[], beforeDate: string, currentSession?: WorkSession) { return sessions.filter((item) => item.project_id === projectId && item.id !== currentSession?.id && !item.is_running && item.time_category === 'billable').filter((item) => item.session_date < beforeDate || (item.session_date === beforeDate && (!currentSession || item.created_at.localeCompare(currentSession.created_at) < 0 || (item.created_at === currentSession.created_at && item.id.localeCompare(currentSession.id) < 0)))).reduce((sum, item) => sum + Number(item.commission_amount), 0); }
