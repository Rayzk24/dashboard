...
describe('règles freelance', () => {
  ...
  it('plafonne la commission cumulée de la mission', () => {
    expect(sessionAmounts(300, 12, 20, 'billable', 50, 45)).toEqual({ gross: 60, commission: 5, net: 55 });
  ...
  it('considère une session commissionnée payée dès que son net est couvert', () => {
    const commissioned = { ...session, gross_amount: 36, commission_amount: 7.2, net_amount: 28.8 };
    const allocations = [{ id: 'a-net', user_id: 'u1', payment_id: 'p-net', work_session_id: commissioned.id, allocated_amount: 28.8 }] as PaymentAllocation[];
    expect(remainingForSession(commissioned, allocations)).toBe(0);
    expect(deterministicAllocationPlan([commissioned], [paymentOf('p-net', 28.8, '2026-07-15')])).toEqual([{ paymentId: 'p-net', workSessionId: commissioned.id, amount: 28.8 }]);
  });
